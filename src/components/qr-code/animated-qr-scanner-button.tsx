...
export default function AnimatedQRCodeScannerButton({
	onResult,
	...props
}: { onResult: (data: string) => void } & Omit<
	IconButtonProps,
	"icon" | "aria-label"
>) {
	...
	// 		// const normal = subject.pipe(
	// 		// 	filter((part) => !part.startsWith("ur:bytes")),
	// 		// );
						...
						<ModalContent>
							<ModalBody p="2">
								<BarcodeScannerComponent
									...
									onUpdate={(err, result) => {
										if (subject && result && result.getText())
											subject.next(result.getText());
									}}
									onError={(err) => {
										if (!subject) return;
							...
							<ModalFooter px="2" pb="2" pt="0" alignItems="center" gap="2">
								{progress !== undefined && (
									<Progress hasStripe value={progress * 100} w="full" />
								)}
