...
export function renderSpotifyUrl(match: URL) {
	...
	const isList =
		match.pathname.startsWith("/album") ||
		match.pathname.startsWith("/playlist");
...
export function renderTidalUrl(match: URL) {
	...
	const [_, _browse, type, id] =
		match.pathname.match(/(\/browse)?\/(track|album)\/(\d+)/i) ?? [];
...
export function renderStemstrUrl(match: URL) {
	...
	return (
		<EmbedEventPointerCard
			pointer={{ type: "nevent", data: { id, relays: [STEMSTR_RELAY] } }}
		/>
	);
}
...
export function renderSoundCloudUrl(match: URL) {
	if (
		match.hostname !== "soundcloud.com" ||
		match.pathname.split("/").length !== 3
	)
		return null;
