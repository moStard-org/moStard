...
const db = await openDB<SchemaV13>(dbName, version, {
	upgrade(db, oldVersion, newVersion, transaction, event) {
		if (oldVersion < 1) {
			...
			userFollows.createIndex("follows", "follows", {
				multiEntry: true,
				unique: false,
			});
		...
		if (oldVersion < 2) {
			const trans = transaction as unknown as IDBPTransaction<
				SchemaV1,
				string[],
				"versionchange"
			>;
		...
		if (oldVersion < 7) {
			const transV6 = transaction as unknown as IDBPTransaction<
				SchemaV6,
				string[],
				"versionchange"
			>;
			const transV7 = transaction as unknown as IDBPTransaction<
				SchemaV7,
				string[],
				"versionchange"
			>;
