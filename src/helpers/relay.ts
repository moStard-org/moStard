...
import type { ConnectionState } from "../services/pool";
...
export function splitNostrFilterByPubkeys(
	...
	return splitQueryByPubkeys(filter, relayPubkeyMap);
}

export function splitQueryByPubkeys(
	query: Filter,
	relayPubkeyMap: Record<string, string[]>,
) {
	...
	for (const [relay, pubkeys] of Object.entries(relayPubkeyMap)) {
		if (query.authors || query["#p"]) {
			...
			if (query.authors)
				filtersByRelay[relay].authors = query.authors
					.filter((p) => !allPubkeys.has(p))
					.concat(pubkeys);
			if (query["#p"])
				filtersByRelay[relay]["#p"] = query["#p"]
					.filter((p) => !allPubkeys.has(p))
					.concat(pubkeys);
...
export function getConnectionStateColor(
