...
export function pointerMatchEvent(
	event: NostrEvent,
	...
	if (isReplaceable(event.kind)) {
		if (Reflect.has(pointer, "pubkey")) {
			...
			return (
				event.pubkey === p.pubkey && event.kind === p.kind && d === p.identifier
			);
		}
	} else {
...
export function filterTagsByContentRefs(
	content: string,
	tags: string[][],
	referenced = true,
) {
 ...
 */
export function replaceOrAddSimpleTag(
	draft: EventTemplate,
	tagName: string,
	value: string,
) {
