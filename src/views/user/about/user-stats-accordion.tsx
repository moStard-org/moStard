...
export default function UserStatsAccordion({ pubkey }: { pubkey: string }) {
	...
	const followerCount = useEventCount({
		"#p": [pubkey],
				...
				<AccordionPanel pb="2">
					<StatGroup gap="4" whiteSpace="pre">
						<Stat>
							...
							<StatNumber>
								{contacts ? getPubkeysFromList(contacts).length : "Unknown"}
							</StatNumber>
							{contacts && (
								<StatHelpText>
						...
						{stats && (
							<>
								<Stat>
									...
									<StatNumber>{followerCount || 0}</StatNumber>
								</Stat>
							...
							{stats.zaps_sent && (
								<>
									...
									<Stat>
										...
										<StatNumber>{stats.zaps_sent.msats / 1000}</StatNumber>
									</Stat>
									<Stat>
										<StatLabel>Avg Zap Sent</StatLabel>
										<StatNumber>{stats.zaps_sent.avg_msats / 1000}</StatNumber>
									</Stat>
									<Stat>
										<StatLabel>Biggest Zap Sent</StatLabel>
										<StatNumber>{stats.zaps_sent.max_msats / 1000}</StatNumber>
									</Stat>
								</>
							)}
							...
							{stats.zaps_received && (
								<>
									...
									<Stat>
										...
										<StatNumber>{stats.zaps_received.msats / 1000}</StatNumber>
									</Stat>
									<Stat>
