...
const NotificationsTimeline = memo(
	({
		...
		for (const event of timeline) {
			if (event.created_at < dates.cursor && filtered.length > minItems)
				continue;
		...
		for (const event of filtered) {
			...
			if (prev.diff(dayjs.unix(event.created_at), "d") > 0) {
				...
				ids = [];
				items.push(
					<TimeMarker key={prev.unix() + "-marker"} date={prev} ids={ids} />,
				);
			}

			const visible =
				event.created_at <= dates.max && event.created_at >= dates.min;
			ids.push(event.id);
			items.push(
				<NotificationItem
					key={event.id}
					...
					visible={visible}
					minHeight={
						visible ? undefined : numberCache.get(getEventUID(event)) + "px"
					}
				/>,
			);
		}
...
function NotificationsPage() {
	...
	const setFocused = useCallback(
		(id: string) => cachedFocus.next(id),
		[cachedFocus],
	);
	const focusContext = useMemo(
		() => ({ id: focused, focus: setFocused }),
		[focused, setFocused],
	);

	const showReplies = useLocalStorageDisclosure(
		"notifications-show-replies",
		true,
	);
	const showMentions = useLocalStorageDisclosure(
		"notifications-show-mentions",
		true,
	);
	const showQuotes = useLocalStorageDisclosure(
		"notifications-show-quotes",
		true,
	);
	const showReposts = useLocalStorageDisclosure(
		"notifications-show-reposts",
		true,
	);
	const showReactions = useLocalStorageDisclosure(
		"notifications-show-reactions",
		false,
	);
	const showUnknown = useLocalStorageDisclosure(
		"notifications-show-unknown",
		false,
	);
