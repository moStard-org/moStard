		...
		<SimpleView title="App Relays" maxW="6xl">
			...
			{sorted.map((url) => (
				<RelayControl
					key={url}
					url={url}
					onRemove={() => removeAppRelay(url, RelayMode.BOTH)}
				>
					<Button
			...
			<Text fontStyle="italic" color="gray.500">
				These are the recommended relays presets for the app. They are used for
				everything in the app.
			</Text>
			<SimpleGrid columns={{ base: 1, lg: 2, xl: 3 }} spacing="2">
				<RelaySetCard
					label="Popular Relays"
					read={RECOMMENDED_RELAYS}
					write={RECOMMENDED_RELAYS}
				/>
				<RelaySetCard
					label="Japanese relays"
					read={RECOMMENDED_JAPANESE_RELAYS}
					write={RECOMMENDED_JAPANESE_RELAYS}
				/>
			</SimpleGrid>
			...
			<Text fontStyle="italic" color="gray.500">
				Index (or lookup) relays are special indexing relays that are used to
				find user profiles, user mailboxes, and other important information
			</Text>
			...
			{lookupRelays.map((url) => (
				<RelayControl
					...
					url={url}
					onRemove={() => {
						localSettings.lookupRelays.next(
							lookupRelays.filter((r) => r !== url),
						);
					}}
				/>
			))}
			...
			{lookupRelays.length === 0 && (
				<Text color="yellow.500">
					<WarningIcon /> There are no index relays set, profile lookup and
