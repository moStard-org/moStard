...
function InboxRelay({ url }: { url: string }) {
	...
	const remove = useAsyncAction(async () => {
		await actions
			.exec(RemoveInboxRelay, url)
			.forEach((e) => publish("Remove inbox relay", e));
	});
				...
				<>
					{info?.limitation?.restricted_writes && (
						<Text color="orange.500" fontSize="sm">
							Restricted writes: May cause issues with other users sending you
							notes
						</Text>
					)}
...
function OutboxRelay({ url }: { url: string }) {
	...
	const remove = useAsyncAction(async () => {
		await actions
			.exec(RemoveOutboxRelay, url)
			.forEach((e) => publish("Remove outbox relay", e));
	});
...
function MailboxesPage() {
	...
	const addInboxRelay = useAsyncAction(async (relay: string) => {
		await actions
			.exec(AddInboxRelay, relay)
			.forEach((e) => publish("Add inbox relay", e));
	});

	const addOutboxRelay = useAsyncAction(async (relay: string) => {
		await actions
			.exec(AddOutboxRelay, relay)
			.forEach((e) => publish("Add outbox relay", e));
	});
	...
	return (
		...
		>
			<Text fontStyle="italic" mt="-2">
				Mailbox relays are a way for other users to find your events, or send
				you events. they are defined in{" "}
				<Link
					color="blue.500"
					isExternal
			...
			{(mailboxes?.inboxes?.length ?? 0) > 4 && (
				<Alert status="warning" variant="subtle" mt="2">
					<AlertIcon />
					Having too many inbox relays will make it difficult for other users to
					send you notes
				</Alert>
			)}
			...
			{(mailboxes?.outboxes?.length ?? 0) > 4 && (
				<Alert status="warning" variant="subtle" mt="2">
					<AlertIcon />
					Having too many outbox relays will make it difficult for other users
					to find your notes
				</Alert>
			)}
