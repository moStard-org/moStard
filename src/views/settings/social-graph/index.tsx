...
function FollowDistanceGroup({
	...
	const users = useObservableEagerMemo(
		() =>
			socialGraph$.pipe(
				map((graph) => graph.getUsersByFollowDistance(distance)),
			),
		[distance],
	);
...
function SocialGraphCronSettings() {
	...
	const lastUpdated = useObservableEagerState(
		localSettings.lastUpdatedSocialGraph,
	);
				...
				<HStack spacing={4} wrap="wrap">
					<Flex alignItems="center" gap={2}>
						...
						<Select
							...
							onChange={(e) => {
								...
								localSettings.updateSocialGraphInterval.next(
									hours * 1000 * 60 * 60,
								);
							}}
							w="auto"
					...
					<Flex alignItems="center" gap={2}>
						...
						<Select
							value={distance}
							onChange={(e) =>
								localSettings.updateSocialGraphDistance.next(
			...
			<VStack alignItems="flex-end" spacing={2} ms="auto">
				{running ? (
					<Flex alignItems="center" gap={2}>
						...
						<Button
							colorScheme="red"
							onClick={() => updateSocialGraphCron.cancel()}
						>
							Cancel
						</Button>
					</Flex>
...
export default function SocialGraphSettings() {
	...
	const root = useObservableEagerMemo(
		() => socialGraph$.pipe(map((graph) => graph.getRoot())),
		[],
	);
	const size = useObservableEagerMemo(
		() => socialGraph$.pipe(map((graph) => graph.size())),
		[],
	);
	...
	return (
		<SimpleView title="Social Graph" maxW="container.xl">
					...
					<Card direction="column" gap={4} p={4} rounded="md">
						...
						{!size || size.users <= 1 ? (
							...
							<>
								<FollowDistanceGroup
									distance={1}
									max={displayMaxPeople}
									label="Friends (1st degree)"
								/>
								<FollowDistanceGroup
									distance={2}
									max={displayMaxPeople}
									label="Friends of Friends (2nd degree)"
								/>
								<FollowDistanceGroup
									distance={3}
									max={displayMaxPeople}
						...
						>
							<Flex gap="2">
								...
								<Button
									type="submit"
									flexShrink={0}
									isLoading={downloadGraph.loading}
								>
									Download
								</Button>
							</Flex>
							<Text fontSize="sm" color="gray.500">
								A URL to download serialized social graph that is compatible
								with the{" "}
								<Link
									href="https://github.com/mmalmi/nostr-social-graph"
									isExternal
									color="blue.500"
								>
									nostr-social-graph
			...
			<Text fontSize="sm" color="gray.500">
				Configure automatic updates to keep your social graph current with new
