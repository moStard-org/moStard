...
export default function PicturePostForm({
	onSubmit,
	...
	const showAdvanced = advanced.isOpen || getValues("nsfw");
						...
						<ErrorBoundary key={media.id}>
							<MediaSlide
								...
								onChange={(alt) =>
									setValue(
										"media",
										getValues("media").map((m) =>
											m.id === media.id ? { ...media, alt } : m,
										),
										setOptions,
									)
								}
				...
				<NewMediaSlide
					onSelect={(files) =>
						setValue(
							"media",
							[
								...files.map((file) => ({ id: nanoid(6), file })),
								...getValues("media"),
							],
							setOptions,
						)
