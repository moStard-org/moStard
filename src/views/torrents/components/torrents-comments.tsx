...
export const ThreadPost = memo(
	({ post, level = -1 }: { post: ThreadItem; level?: number }) => {
		const { showReactions } = useAppSettings();
		const expanded = useDisclosure({
			defaultIsOpen: level < 2 || post.replies.size <= 1,
		});
		const replyForm = useDisclosure();

		const muteFilter = useClientSideMuteFilter();

		const replies = Array.from(post.replies).filter(
			(r) => !muteFilter(r.event),
		);
...
export default function TorrentComments({ torrent }: { torrent: NostrEvent }) {
	...
	const { timeline } = useThreadTimelineLoader(torrent, readRelays, [
	...
	return (
		<IntersectionObserverProvider callback={callback}>
			{thread?.root &&
				repliesByDate(thread.root).map((item) => (
					<ThreadPost key={item.event.id} post={item} level={0} />
				))}
