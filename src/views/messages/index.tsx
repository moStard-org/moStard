...
function MessagePreview({ message }: { message: NostrEvent }) {
	...
	return plaintext ? (
		<Text isTruncated>{plaintext || "<Encrypted>"}</Text>
	) : (
		<Badge variant="subtle">Encrypted</Badge>
	);
}
...
function ConversationCard({
	...
	style,
	data,
}: ListChildComponentProps<(LegacyGroup | WrappedGroup)[]>) {
...
function Groups() {
	...
	const { loader } = useTimelineLoader(
		`${account.pubkey}-legacy-messages`,
		legacyInboxes ?? [],
		[
	...
	const groups = useMemo(() => {
		...
		const all: (LegacyGroup | WrappedGroup)[] = [
			...(legacyGroups ?? []),
			...(wrappedGroups ?? []),
		];
		...
		for (const group of all) {
			...
			if (existing) {
				if (existing.lastMessage.created_at < group.lastMessage.created_at)
