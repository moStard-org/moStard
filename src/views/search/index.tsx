...
export function SearchPage() {
	...
	const relay =
		params.get("relay") ??
		(localSearchSupported ? "" : undefined) ??
		searchRelays[0] ??
		"";
	...
	const handleSearchText = (text: string) => {
		...
		if (
			cleanText.startsWith("nostr:") ||
			cleanText.startsWith("web+nostr:") ||
			safeDecode(text)
		) {
			navigate(
				{ pathname: "/l/" + encodeURIComponent(text) },
				{ replace: true },
			);
			return true;
		}
		...
		if (hashTagMatch) {
			navigate(
				{ pathname: "/t/" + hashTagMatch[2].toLocaleLowerCase() },
				{ replace: true },
			);
			return true;
		}
	...
	const shouldSearch =
		!!searchQuery && (!!relay || (localSearchSupported && !!cacheRelay));
	...
	return (
		<VerticalPageLayout>
			...
			<Flex direction="column" gap="2">
				{shouldSearch ? (
					<SearchResults relay={relay} query={searchQuery} />
				) : null}
			</Flex>
		</VerticalPageLayout>
	);
