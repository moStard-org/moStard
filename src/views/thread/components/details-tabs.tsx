...
export default function DetailsTabs({ post }: { post: ThreadItem }) {
	...
	const reposts = events.filter(
		(e) => e.kind === kinds.Repost || e.kind === kinds.GenericRepost,
	);
	const quotes = events.filter((e) => {
		...
		return (
			...
			getContentTagRefs(e.content, e.tags).some(
				(t) => t[0] === "e" && t[1] === post.event.id,
			)
		);
	});
	...
	return (
		...
		>
			...
			<TabPanels minH={{ base: "50vh", md: "0" }}>
				{tabs.filter((t) => t.visible).map((tab) => tab.element)}
			</TabPanels>
		</Tabs>
	);
}
